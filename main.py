...
def func(message):
    if message.chat.id == ID:
        if (message.text == "Отримати скріншот екрану"):
            ...
            snd_message = bot.send_message(ID, "Введіть команду, яка виконається в командному рядку", reply_markup=markup_cmd)
            ...
            btn2 = types.KeyboardButton("Натиснути на праву клавішу миші")
            back_back = types.KeyboardButton("Назад")
            markup_command.add(btn1, btn2, back_back)
        ...
        if message.text == "Пересунути курсор":
            global move_cur
            ...
            send_message = bot.send_message(ID, "Введіть куди пересунути курсор (x, y) через кому. Приклад: 100,100:", reply_markup=reply_curcor)
            bot.register_next_step_handler(send_message, move_cursor)
        elif message.text == "Натиснути на праву клавішу миші":
            reply_curcor = types.InlineKeyboardMarkup(row_width=1)
            click1 = types.InlineKeyboardButton("Натиснути", callback_data="click1")
            back2 = types.InlineKeyboardButton("Скасування", callback_data="back2")
            reply_curcor.add(click1, back2)
            bot.send_message(ID,
                                            "Натисніть на кнопку",
